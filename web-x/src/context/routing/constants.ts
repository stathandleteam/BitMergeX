export const ROUTES = {
  AppUX: '/app-ux',
  HOME: '/home',
  ONBOARDING: '/onboarding',
  SIGNUP: '/signup',
  LOGIN: '/login',
  DASHBOARD: '/dashboard',
  SETTINGS: '/settings',
  SEED_PHRASE_CREATE: '/seed-phrase-create',
  SEED_PHRASE_RECOVER: '/seed-phrase-recover',
  SEED_PHRASE_CONFIRM: '/seed-phrase-confirm',
  PASSWORD_SECURITY: '/password-security',
  WALLET_CREATED: '/wallet-created',
  RECOVERY_STEP_ONE: '/recovery-step-one',
  ACCOUNT_LIST: '/account-list',
  STX_DETAILS: '/stx-details-and-history',
  BTC_DETAILS: '/btc-details-and-history',
  TRANSFER_STX: '/transfer-stx',
  NETWORK_SETTINGS_SCREEN: '/network-settings',
  PRIVATE_KEY_SCREEN: '/private-key-settings',
  HELP_SUPPORT_SETTINGS_SCREEN: '/help-support',
  TRANSACTION_SENT_SCREEN: '/transaction-sent-screen',
} as const;

export type RouteKeys = keyof typeof ROUTES;
export type Routes = typeof ROUTES[RouteKeys];