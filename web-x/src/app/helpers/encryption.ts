...
export const encryptSeed = (secretKey: string, password: string): Promise<string> => {
  return Promise.resolve(AES.encrypt(secretKey, password).toString());
}

export const decryptSeed = (encryptedKey: string, password: string): Promise<string | null> => {
  try {
    const bytes = AES.decrypt(encryptedKey, password);
    return Promise.resolve(bytes.toString(enc.Utf8));
  } catch (e) {
    return Promise.reject(null); // Decryption failed, likely due to incorrect password
  }
}